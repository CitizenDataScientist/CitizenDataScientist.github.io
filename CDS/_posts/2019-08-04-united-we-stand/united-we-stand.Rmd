...
plot_bkgd <- "#f2f2ef"  # plot background colour #F3F3EF
plot_grid <- "#D0CEC5"  # plot girdlines colour
plot_font <- "Lato"     # plot font family
...
* **Which _sentiments_ underlie the debate over the topic of climate change?**
...
* **Session** : : General Assembly session number, from 25 to 70;
* **Year** : : year of the corresponding session, from 1970 to 2015;
* **Country** : : representatives' country code^[ISO 3166 Alpha-3 country code, more information in the following link: https://www.iso.org/iso-3166-country-codes.html];
* **Text** : : complete text of each countries' statement at the UN General Debate, for every year in the dataset;

The analysis was conducted in **R**, following the _tidytext_ principles defined by Julia Silge and David Robinson in the book _"Text Mining with R"_ [@silge2016tidytext]. The R Markdown file containing the fully reproducible source code is hosted on Github.
...
speech_median_length %>%
  ...
  # configure scales and set labels
  scale_y_continuous(breaks = seq(0, 12000, 2000), limits = c(0, 12500), expand = c(0, 0), labels = scales::comma_format()) +
  labs(title = 'Top and bottom 8 countries, by median word count', subtitle = 'some countries go on forever, others not so much',
       x = '', y = 'word count') + 
  # plot-specific theme 
...
<aside>
each dot represents a speech; median is depicted by the blue diamond
</aside>
...
Concerning the task of confining the segments of each speech pertaining to climate change, the decision was made to extend the scope of the analysis to also include the subsequent sentence in the vicinity of those containing mentions of climate change. The outcome of this process, which effectively creates a _window of interest_ of two sentences per mention of climate change, is illustrated below^[rendered using the `ggpage` package - available on this link: https://emilhvitfeldt.github.io/ggpage/].
...
<aside>
Dutch speech delivered in 1989, the first to mention _climate change_ twice
</aside>

```{r sentimentr_generate_highlight, include=FALSE}
climate_window_new <- climate_window %>% 
  # filter(hit %in% c("match", "following")) %>% 
...
# highlight_speeches <- c(3875, # PNG 1996 - "risk of extiction"
#                         4205, # MDV 1998 - "risk of extiction" pt.2
#                         4998, # TUV 2002
#                         5868, # LAO 2007
#                         6186, # CAN 2009 
#                         6595, # EU 2011 - lowest polarity score
#                         7017, # KHM 2013 - highest polarity score
#                         7443) # NLD 2015 - positive polarity (tech. transfer)
...
# take .png a snapshot of the hmtl url (edited offline)
# webshot::webshot(url = "sentence_highlight.html", file = "sentence_highlight.png", zoom = 2)
```

**SentimentR**’s sentiment polarity algorithm was thus employed at sentence level within the speech segments enclosed in the _window of interest_. The overall polarity score of a country’s statement on climate change, issued on a particular year, was computed as the average value assigned to each individual sentence enclosed in the _window of interest_.  Five instances were chosen to portray the unbounded score attributed to the remarks concerning this topic, professed on five different sessions of the General Debate:

* **Papua New Guinea, 1996** : : a _negative_ statement, scored as **-0.368**, which mentions the _risk of extinction_ faced by island-countries in the south-western Pacific;

* **Marshall Islands, 2002** : : a _nuanced_ statement, scored as **-0.223**, in which the contribution of the _negative_ sentence overshadows that of the _positive_. The speaker reiterates that island nations such as the Marshall Islands face the _very threat of complete obliteration_, should the oceans rise by _even marginal levels_.;

* **Timor-Leste, 2007** : :  a _nuanced_ statement, scored as **0.186**, in which the contribution of the _positive_ sentence overshadows that of the _negative_. While praising the ongoing negotiations and joint efforts to address the matter of climate change, the speaker emphasizes how developed countries in the northern hemisphere have damaged the _fragile ecosystems_ of the region.;

* **Burkina Faso, 2010** : : most _positive_ statement on record, scored as **1.055**;

* **Singapore, 2014** : : most _negative_ statement on record, scored as **-0.700**;

```{r sentimentr_render_highlight, echo=FALSE, fig.height=8, fig.width=16, warning=FALSE, dev='svg', layout="l-page"}
...
Each point, representing the score of a given country's statement on a given year, is colour-coded according to total number of mentions of climate change, introduced in previous visualizations. Every year a polarizing array of statements is made, with the resulting average yearly score being generally **favourable** and **stable**. The same data is depicted in the plot below, facetted by cluster of total mentions, in an attempt to assess if the trend exhibits meaningful differences on any specific group of countries.
...
* countries than refrain from addressing the topic (5 or less total mentions) do it in _lenient fashion_;
* sentiment towards climate change for the majority of countries at the UN General Debate (ranging from 6 to 50 mentions of the topic) remains positive on average, but with declining trend. Countries in this cluster also make the most _polarizing_ statements;
* countries that recognize the relevance of the problem (having 51 to 100 total mentions) also issue _polarizing_ statements, but with less pronounced trends;
* countries with the highest number of mentions (over 100) - despite exhibiting a negative sentiment trend - remain favourable and do not profess remarkably negative statements, as their sense of urgency towards the topic would lead us to believe;
